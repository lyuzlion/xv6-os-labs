...
struct contect {
  uint64 ra; // 表示函数调用返回地址。这是当一个函数完成执行后，程序控制流应该返回的位置。
  uint64 sp;
...
// M: we should modify the thread structure to 
// include the stack and state of the thread
// which helps 
struct thread {
  char       stack[STACK_SIZE]; /* the thread's stack */
  int        state;             /* FREE, RUNNING, RUNNABLE */
  struct contect context;
};
struct thread all_thread[MAX_THREAD];
struct thread *current_thread;
// extern void thread_switch(uint64, uint64);
extern void thread_switch(struct contect*, struct contect*);
